...
def count_down(count):
    ...
    count_sec = count % 60
    format_min = count_min if count_min > 9 else f"0{count_min}"
    format_sec = count_sec if count_sec > 9 else f"0{count_sec}"
    text = f"{format_min}:{format_sec}"
...
title_label = tk.Label(text="Timer", fg=GREEN, font=(
...
reset_button = tk.Button(
    text="Reset", highlightthickness=0, command=rest_timer)
